...
class TrainSimulationService {
  final Map<String, double> _trainProgress =
      {}; // Progreso de cada tren (0.0 a 1.0)
  final Map<String, bool> _trainDirection =
      {}; // true = avanzando, false = retrocediendo
  final Map<String, List<StationModel>> _lineStations = {};
  bool _isTestMode =
      false; // En modo test, los trenes se mueven con tiempo acelerado
  ...
  static const double speedFactor =
      0.8; // 80% de la velocidad normal (más suave y visible)
  ...
  void initialize(List<StationModel> stations) {
    ...
    for (var train in sampleTrains) {
      ...
      if (lineStations.isNotEmpty) {
        ...
        _trainProgress[train.id] =
            _calculateInitialProgress(train, lineStations);
  ...
  double _calculateInitialProgress(
      TrainModel train, List<StationModel> stations) {
  ...
  void _updateTrainPositions() {
    ...
    for (var train in sampleTrains) {
      ...
      if (_isTestMode) {
        ...
        timeFactor =
            20.0; // Aceleración 20x (1 segundo real = 20 segundos simulados)
      } else {
        // Modo normal
        baseSpeed = (train.velocidad / 100.0) * speedFactor;
        timeFactor = 1.0;
      ...
      final progressIncrement =
          baseSpeed * (updateInterval.inSeconds / 60.0) * timeFactor;
  ...
  GeoPoint getTrainPosition(TrainModel train) {
    ...
    if (lineStations == null || lineStations.length < 2) {
      return train
          .ubicacionActual; // Retornar posición original si no hay estaciones
    }
    ...
    final segmentProgress =
        (clampedProgress * segmentCount) - finalSegmentIndex;
    ...
    final lat = startStation.ubicacion.latitude +
        (endStation.ubicacion.latitude - startStation.ubicacion.latitude) *
            finalSegmentProgress;
    final lng = startStation.ubicacion.longitude +
        (endStation.ubicacion.longitude - startStation.ubicacion.longitude) *
            finalSegmentProgress;
