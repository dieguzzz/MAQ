...
class FirebaseService {
  ...
  Future<void> createUser(UserModel user) async {
    try {
      ...
      throw Exception(
          'Error al crear usuario: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<void> updateUser(String uid, Map<String, dynamic> data) async {
    try {
      ...
      throw Exception(
          'Error al actualizar usuario: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<List<StationModel>> getStations() async {
    ...
    return snapshot.docs.map((doc) => StationModel.fromFirestore(doc)).toList();
  }
  ...
  Stream<List<TrainModel>> getTrainsStream() {
    return _firestore.collection('trains').snapshots().map((snapshot) =>
        snapshot.docs.map((doc) => TrainModel.fromFirestore(doc)).toList());
  }
  ...
  Future<String> createReport(ReportModel report) async {
    try {
      ...
      if (reportData['usuario_id'] == null ||
          reportData['usuario_id'].toString().isEmpty) {
        throw Exception('El ID de usuario es requerido');
      }
      if (reportData['objetivo_id'] == null ||
          reportData['objetivo_id'].toString().isEmpty) {
      ...
      throw Exception(
          'Error al crear reporte: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<void> _confirmReportInternal(String reportId, String userId) async {
    try {
      ...
      final reportUserId =
          (reportData['userId'] ?? reportData['usuario_id']) as String?;
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        final currentConfirmations = transactionReportData['confirmations'] ??
            transactionReportData['confirmation_count'] ??
            0;
      ...
      await gamificationService.awardPointsToReportAuthor(
      ...
      throw Exception(
          'Error al confirmar reporte: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<UserCredential> signInWithEmailAndPassword(
    ...
    try {
      ...
      throw Exception(
          'Error al iniciar sesión: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<UserCredential> createUserWithEmailAndPassword(
    ...
    try {
      ...
      throw Exception(
          'Error al crear cuenta: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<UserCredential> signInAnonymously() async {
    try {
      ...
      throw Exception(
          'Error al iniciar sesión como invitado: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Future<UserCredential?> signInWithGoogle() async {
    try {
      ...
      throw Exception(
          'Error al iniciar sesión con Google: ${ErrorHandlerService.getErrorMessage(e)}');
    }
  }
  ...
  Stream<List<LearningReportModel>> getLearningReportsStream({int days = 14}) {
  ...
  Future<void> updateModelMetrics(Map<String, dynamic> metrics) async {
    try {
      await modelMetricsRef
          .doc('current')
          .set(metrics, SetOptions(merge: true));
    } on FirebaseException catch (e) {
      throw Exception(ErrorHandlerService.getErrorMessage(e));
    } catch (e) {
      throw Exception(
