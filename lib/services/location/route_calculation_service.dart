...
class RouteCalculationService {
  ...
  static List<StationModel> calculateRoute(
    ...
    try {
      ...
      connectionStation = allStations.firstWhere(
        (s) =>
            s.linea == origen.linea &&
            (s.id == 'l1_san_miguelito' ||
    ...
    final combinedRoute = <StationModel>[
      ...routeToConnection,
      ...routeFromConnection
          .skip(1), // Saltar la estación de conexión duplicada
    ];
  ...
  static List<StationModel> _calculateDirectRoute(
    ...
    final lineStations =
        allStations.where((s) => s.linea == origen.linea).toList()
          ..sort((a, b) {
    ...
    if (origenIndex < destinoIndex) {
      ...
      return lineStations
          .sublist(destinoIndex, origenIndex + 1)
          .reversed
          .toList();
    }
  }
