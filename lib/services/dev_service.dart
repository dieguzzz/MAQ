...
class DevService {
  static final ValueNotifier<bool> _devModeNotifier = ValueNotifier<bool>(false);
  ...
  static final LearningReportService _learningReportService = LearningReportService();
  ...
  static Future<void> simulateArrivalReport({
    ...
    try {
      ...
      final devUserId = userId ?? 'dev_user_${DateTime.now().millisecondsSinceEpoch}';
  ...
  static Future<void> _updateMetrics() async {
    try {
      ...
      final todayReports = await _firestore
          ...
          .where('creado_en', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay))
          .get();
      ...
      final accuracy = totalReports > 0 
          ? (accurateReports / totalReports) * 100.0 
          : 0.0;
  ...
  static Future<List<Map<String, dynamic>>> _getProblemStations() async {
    try {
      ...
      for (final station in stations) {
        ...
        if (reports.docs.isNotEmpty) {
          ...
          if (delays.isNotEmpty) {
            ...
            if (avgDelay > 5) { // Más de 5 minutos de retraso promedio
              problemStations.add({
                'id': station.id,
                'name': station.nombre,
      ...
      problemStations.sort((a, b) => 
          (b['avgDelay'] as int).compareTo(a['avgDelay'] as int));
